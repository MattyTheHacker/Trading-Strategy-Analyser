...
def render_register(findings: list[Finding]) -> str:
    ...
    out = [
        *_header(),
        ...
        "**that archetype's campaign database and the `tools/campaign_*.py` the file names -- not a**",
        "**standing property.** Quote the file rather than any summary of it. The databases are not",
        "committed and two sets exist, one per side of the re-sweep --",
        '`docs/findings/README.md` § "Reproducing any figure here".',
        "",
