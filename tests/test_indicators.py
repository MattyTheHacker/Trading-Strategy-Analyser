...
def test_nt8_ema_matches_the_hand_computed_recursion():
...
def test_nt8_ema_differs_from_talib_which_is_why_it_exists():
...
def test_nt8_ema_seeding_difference_decays_with_distance():
...
def test_nt8_sma_averages_a_partial_window_during_warm_up():
...
def test_nt8_sma_is_a_true_rolling_mean_once_warmed_up():
    out = indicators.nt8_sma(RAMP, 3)
...
def test_nt8_sma_recursion_does_not_drift_meaningfully_over_a_long_series():
...
def test_moving_averages_handle_short_and_empty_inputs(period):
...
def test_session_vwap_accumulates_within_a_session():
...
def test_session_vwap_reanchors_at_each_session_open():
...
def test_session_vwap_survives_zero_volume_bars():
...
def test_new_session_flags_marks_each_days_first_bar():
...
def test_new_session_flags_on_empty_input():
