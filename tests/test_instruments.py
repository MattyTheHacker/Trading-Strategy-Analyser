...
def test_tick_values_differ_tenfold_between_nq_and_mnq():
...
def test_point_and_tick_conversions_round_trip():
...
def test_round_to_tick(price, mode, expected):
    assert MNQ.round_to_tick(price, mode) == pytest.approx(expected)


def test_round_to_tick_handles_arithmetic_residue():
...
def test_position_size_rounds_down_and_can_refuse_the_trade():
...
def test_position_size_rejects_nonpositive_stop():
    with pytest.raises(ValueError):
        MNQ.position_size_for_risk(100.0, 0.0)


def test_contract_id_parses_nt8_export_names():
...
def test_contract_ids_sort_into_expiry_order():
...
def test_contract_id_rejects_non_quarterly_months():
    with pytest.raises(ValueError, match="quarterly"):
        ContractId(year=2024, month=1, root="MNQ")


def test_contract_id_rejects_unparseable_names():
    with pytest.raises(ValueError, match="cannot parse"):
        ContractId.parse("MNQ-03-24")


def test_unknown_instrument_names_the_known_ones():
