...
class SqueezeBreakoutParams:
    """Rule set for the SqueezeBreakout archetype -- an original, with no NinjaScript.
    ...
    squeeze_form: int = int(compression.CompressionForm.BANDWIDTH)
    """Which width measure the squeeze ranks -- one of :class:`nqbt.compression.CompressionForm`."""

    squeeze_period: int = 20
    """Bars the width is measured over, which is also the window whose high and low the order
    rests beyond."""

    squeeze_baseline_bars: int = 250
    """Bars before each one that its width is ranked against."""

    squeeze_below: float = 0.1
    """The squeeze: a width ranked **strictly** below this share of its baseline. ``1.0`` admits
    every bar but one wider than its whole baseline."""

    min_squeeze_bars: int = 1
    """Consecutive bars, the signal bar included, the squeeze must have held."""
    ...
    compression_filter: int = compression.ALL_STATES
    """Compression states an entry may be taken in -- see
    :attr:`DeadCatParams.compression_filter`. A second compression condition beside the squeeze,
    ...
    stop_offset_ticks: int = 2
    """Ticks beyond the stop's level under :data:`ORB_STOP_OPPOSITE` and :data:`ORB_STOP_FRACTION`."""

    stop_range_fraction: float = 0.5
    """How far back across the window the stop sits under :data:`ORB_STOP_FRACTION`, as a fraction
    of its width from the extreme that was broken -- see
    :attr:`OpeningRangeParams.stop_range_fraction`."""
    ...
    min_bracket_dollars: float = 0.0
    """Floor on the ATR stop distance in **dollars per contract**, off at ``0`` -- see
    :attr:`OpeningRangeParams.min_bracket_dollars`."""

    target_mode: int = ORB_TARGET_R
    """One of :data:`ORB_TARGET_MODES`, a width being the window's."""

    target_r_multiples: tuple[float, ...] = (1.0, 1.5, 2.0, float("nan"))
    """Per-leg targets in R, read under :data:`ORB_TARGET_R`. ``nan`` marks a runner."""

    target_width_multiples: tuple[float, ...] = (1.0, float("nan"))
    """Per-leg targets as multiples of the **window's width** past the trigger, read under
    :data:`ORB_TARGET_WIDTH`."""
    ...
    commission_per_contract: float = 0.0
    slippage_ticks: float = 0.0
    """Adverse slippage on the stop entry and both market exits. Never applied to a limit target."""
    ...
    def _validate_squeeze(self) -> None:
        ...
        compression.validate_form(self.squeeze_form)
        compression.validate_period(self.squeeze_period)
        compression.validate_baseline_bars(self.squeeze_baseline_bars)
        if not 0.0 < self.squeeze_below <= 1.0:
            msg = f"squeeze_below must lie in (0, 1], got {self.squeeze_below}; no rank is below 0"
            raise ValueError(msg)

        if self.min_squeeze_bars < 1:
            msg = f"min_squeeze_bars must be >= 1, got {self.min_squeeze_bars}"
            raise ValueError(msg)
    ...
    @property
    def squeeze_key(self) -> compression.CompressionKey:
        """Which of the dataset's compression series the squeeze ranks."""
        return compression.key(self.squeeze_form, self.squeeze_period, self.squeeze_baseline_bars)
    ...
    @property
    def compression_key(self) -> compression.CompressionKey:
        """Which of the dataset's compression series the compression filter reads."""
        return compression.key(
            self.compression_form,
            self.compression_period,
            self.compression_baseline_bars,
