...
class Notes:
    ...
    def __post_init__(self) -> None:
        ...
        missing = [name for name in TEXT_COLUMNS if name not in self.frame.columns]
        if missing:
            msg = f"a sidecar is missing column(s) {missing}; the shape is nqbt.notes.COLUMNS"
        ...
        if not self.frame.index.is_unique:
            repeated = sorted(self.frame.index[self.frame.index.duplicated()].unique().tolist())
            msg = (
...
def record(notes: Notes, trade_id: int, note: str, *, screenshot: str | None = None) -> Notes:
    ...
    text = note.strip()
    if not text:
        msg = f"trade {trade_id}'s note is empty; a trade with nothing to say carries no row"
        raise NotesError(msg)
    ...
    kept = notes.frame.drop(index=int(trade_id), errors="ignore")
...
def read(path: Path | str) -> Notes:
    ...
    missing = [name for name in COLUMNS if name not in raw.columns]
    if missing:
        msg = f"{path}: not a notes sidecar -- missing column(s) {missing}. The shape is nqbt.notes.COLUMNS."
        raise NotesError(msg)
    unknown = [name for name in raw.columns if name not in COLUMNS]
    if unknown:
        msg = (
...
def alongside(frame: pd.DataFrame, notes: Notes) -> pd.DataFrame:
    ...
    **The one join that puts free text beside a number**, and nothing computing a statistic may
    ...
    if KEY not in frame.columns and frame.index.name != KEY:
        ...
        raise NotesError(msg)
    ids = frame[KEY] if KEY in frame.columns else frame.index.to_series()
    attached = frame.copy()
...
def check_excluded(frame: pd.DataFrame, *, what: str) -> None:
    ...
    """
    found = [name for name in TEXT_COLUMNS if name in frame.columns]
    if not found:
        return
    msg = (
...
def _keyed(raw: pd.DataFrame, *, source: str) -> pd.DataFrame:
    ...
    ids = pd.to_numeric(raw[KEY], errors="coerce")
    unreadable = ids.isna() | (ids % 1 != 0)
    if unreadable.any():
        row = int(unreadable.to_numpy().argmax())
        msg = f"{source}: {KEY} must be a whole number on every row; row {row} carries {raw[KEY].iloc[row]!r}"
        raise NotesError(msg)
    frame = raw[list(TEXT_COLUMNS)].copy()
