...
- **The confirmation entry §M34 deferred is still deferred** — a resting stop above the touch bar's high, which is the literature's step 3 in its strong form where `require_turn` is its weak one. Given that the weak form is a coin flip, the strong one is the more interesting test rather than the less. Built and swept since: [§M39](m39-ema-pullback-confirmation-entry.md).
