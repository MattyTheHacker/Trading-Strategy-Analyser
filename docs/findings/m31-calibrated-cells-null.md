...
- **Gate 4 has not been run on the cell that passed.** No walk-forward, no Monte Carlo, no prop-account replay and no exit-exclusion read — §M28.15 is what that would look like, and it took the midday cell through all four without making it a recommendation. §M31.1 has since run them on this cell.
