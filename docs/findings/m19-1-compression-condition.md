...
**It is not the squeeze archetype**, and it does not answer M19. It supplies the condition and the stratification; the entry model M19 describes — a two-sided break of the compression range — is still unbuilt, and §M28's expressibility finding 1 still stands against the two-sided half of it. [§M19.2](m19-2-squeeze-breakout-spec.md) has since built it one side at a time, and a sixth probe scenario has since shown the managed approach refuses the two-sided half outright. What it does is let the campaign ask whether compression is worth an archetype before one is written, which is the order §M28 and §M28.1 established for the opening range and the order the standing rubric asks for.
