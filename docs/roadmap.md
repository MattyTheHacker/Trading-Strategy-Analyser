...
- **A stored campaign row belongs to the archive it was swept on, and a re-read has to prove it is on those bars.** The archive was extended on 2026-09-16, which moved the 60/40 split under every campaign stored before it, so a gate-3 read of an older campaign silently measured a different holdout; §M37 and §M38 each found that only because their plan carried a control that failed to reproduce. `tools/campaign_null.py` now refuses a re-run whose bar range, trade count or net P&L disagrees with what the **test window** stored for the same configuration, the way `tools/campaign_shortlist.py`'s `verify` always has, and the refusal names which end of the series moved ([#320]). **A configuration that window stored no viable row for is warned about rather than refused**, so a shortlist whose counterparts all fall below `MIN_TRADES` there is unchecked. A refusal means the campaign needs re-sweeping, not that the tool is broken.
