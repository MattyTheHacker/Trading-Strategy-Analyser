...
- **A stored campaign row belongs to the archive it was swept on, and a re-read has to prove it is on those bars.** The archive was extended on 2026-09-16, which moved the 60/40 split under every campaign stored before it, so a gate-3 read of an older campaign silently measured a different holdout; §M37 and §M38 each found that only because their plan carried a control that failed to reproduce. `tools/campaign_null.py` now refuses a re-run whose bar range, trade count or net P&L disagrees with what the **test window** stored for the same configuration, the way `tools/campaign_shortlist.py`'s `verify` always has, and the refusal names which end of the series moved ([#320]). **A configuration that window stored no viable row for is warned about rather than refused**, so a shortlist whose counterparts all fall below `MIN_TRADES` there is unchecked. A refusal means the campaign needs re-sweeping, not that the tool is broken. **§M41 measured how far that reaches and found the two roots differ**: MNQ only grew at its tail, so `campaign_shortlist.swept_series` cuts the archive back and recovers the window exactly — every held-out bound and every trade count reproduces — while NQ gained history *earlier* than its tail, which moved the split and which no truncation recovers. Cutting back is therefore worth trying before concluding a campaign is unreadable, and a tool that does has to report which window each cell actually ran on. **`tools/campaign_swept.py` is that machinery generalised** ([#345]): it re-runs a shortlist over the cut-back archive and hands each log to the caller rather than storing it, so `verify` keeps refusing to file a log against a summary it does not match while a gate-4 read still has a book — `tools/campaign_exits.py --rerun` and `tools/campaign_montecarlo.py --rerun` are the callers.
- **A tool that loads raw bars and prepares them without saying so refuses every rule reading an absolute level, and it fails as a crash rather than as a number.** `context.prepare` defaults to `PriceBasis.UNKNOWN` deliberately — § "The build spec's three loose ends" — so the cost of not stating a basis falls on the caller, and it was paid twice before anything checked: `campaign_shortlist.store_logs` ([#330]) and `campaign_null.measure` ([#340]), each blocking a whole gate on EmaCrossover's `round=on` variant. Neither moved a number, which is the only reason the cost was tolerable — the guard fails closed by design. **[#341] stated it at every remaining call site and made forgetting it a test failure rather than a run-time one**: `tests/test_price_basis.py` reads every `context.prepare`, `prepare_for` and `sweep_axes` call under `nqbt/` and `tools/` and fails on one that does not state a basis, with an `EXEMPT` list carrying the reason wherever a call genuinely cannot. **Adding a caller means adding the keyword**, and the audit is the thing to trust rather than a count in prose — the first one here missed six.
...
### M33 — the volume answer belongs to the channel ([#300])
...
### M41 — What the live flatten timing costs ([#346])

**At 5-minute bars the 180 seconds `InsideBarTrailing.cs` sets picks exactly the same bars as the 30 the simulation runs at, so the backtest-to-live divergence is zero by arithmetic rather than small by measurement; a whole bar earlier costs 1.4% of net on MNQ and gains 0.4% on NQ, and a whole 15-minute bar earlier is worth at most 0.040 of held-out profit factor with the sign belonging to the archetype.** Moved to [`docs/findings/m41-flatten-timing.md`](findings/m41-flatten-timing.md).

### M42 — InsideBarTrailing's midday cell through gate 4 ([#345])
...
**Pre-registered and falsified: 1 of EmaCrossover's 16 gold pairs clears against the 4 required, the one that does is the unfiltered cell already known, and the direction inverts — it clears 3 of 6 S&P pairs against gold's 1 of 16.** Moved to [`docs/findings/gold-strata-result.md`](findings/gold-strata-result.md).
...
[#340]: https://github.com/MattyTheHacker/Trading-Strategy-Analyser/issues/340
