...
- **A tool that loads raw bars and prepares them without saying so refuses every rule reading an absolute level, and it fails as a crash rather than as a number.** `context.prepare` defaults to `PriceBasis.UNKNOWN` deliberately — § "The build spec's three loose ends" — so the cost of not stating a basis falls on the caller, and it has now been paid twice: `campaign_shortlist.store_logs` ([#330]) and `campaign_null.measure` ([#340]), each blocking a whole gate on EmaCrossover's `round=on` variant. **State the basis wherever a tool loads bars it knows and then simulates**; six callers still do not ([#341]). Neither occurrence moved a number, which is the only reason the cost was tolerable — the guard fails closed by design.
