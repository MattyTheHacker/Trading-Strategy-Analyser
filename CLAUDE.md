...
- `docs/findings/` — **one file per campaign, and the home of every result.** `README.md` is the hand-written summary: which strategies look best for a prop account and for a regular one, and how much of it to believe. `register.md`, `by-archetype.md` and `by-gate.md` are generated by `tools/findings_index.py` from each file's front matter — regenerate those three, never hand-edit them. Quote the campaign file rather than a summary of it, including `README.md` and the summaries below.
- `docs/roadmap.md` — the reasoning behind the order, the standing rubric, the standing traps and the decision record. Read it before starting anything. It keeps a stub at each `§Mxx` so an existing pointer still resolves, and the stub names the findings file.
...
- **Bar size is the largest lever and the moving averages are nearly inert.** Resolution explains an order of magnitude more profit-factor variance than any period or kind on every archetype, and each one's median configuration loses money at one minute. **The hardest case for the second half of that has now been run and it holds**: EmaPullback's two averages set its entry *and* its stop, and sweeping both kinds crossed leaves ema against sma worth about two points of held-out profitable share with the ordering inverting across the split — §M35, which also found the two axes the archetype invented to be the only ones whose ordering survives it. Tune the bar size and the exit geometry; not periods. **One piece of exit geometry is now measured and ruled out on every archetype at once**: an unconditional cap on how long a trade may last costs profit factor at every rung that binds, monotonically in how tight it is, and the selection window picks a paying rung zero times in seven — quote `docs/findings/m29-maximum-hold-time.md` rather than this line, and note that it tests the unconditional form only. **OpeningRange is the exception that proves which half of that matters**: its stop mode partitions the archetype into a half that passes gate 1 in all ten root x resolution cells and a half that fails it in all ten, and within the half that works the bar size is only the *third* largest axis, behind which side is traded and how long the range is — §M28.1. Read a pooled eta-squared table there with care: an axis inert under one mode carries its default under the other, so the pooled figure reports the mode wearing a disguise.
