...
- `docs/findings/` — **one file per campaign, and the home of every result.** `README.md` is the register, `by-archetype.md` gathers one archetype's whole story and `by-gate.md` says what has survived which check. All three are generated by `tools/findings_index.py` from each file's front matter — regenerate, never hand-edit. Quote the campaign file rather than a summary of it, including the summaries below.
