...
- **Every registered archetype was swept across every axis it owned at §M27** — `docs/roadmap.md` §M27, the section to quote rather than any line here. **InsideBar is the only one that survives held-out selection *and* shows a positive excess over a matched random entry**, and what stops it is its bracket rather than its entry. Two things have been added since: ElasticBand's `band_source`, which clears the first of those two tests and not the second (§M26.4), and the **OpeningRange** archetype, which was registered after that campaign and so is not part of its verdict — §M28.1.
...
- **Bar size is the largest lever and the moving averages are nearly inert.** Resolution explains an order of magnitude more profit-factor variance than any period or kind on every archetype, and each one's median configuration loses money at one minute. Tune the bar size and the exit geometry; not periods. OpeningRange is the first archetype where the **exit geometry alone** separates the results — its stop mode moves the median profit factor further than any other axis it owns, and it is the only one of them whose ordering survives the held-out window.
...
- **A dense entry signal has no matched random-entry null, and that is not a small caveat.** OpeningRange's trigger is a level rather than an event, so it resubmits on every armed bar; `randomentry.matched_random_signal` then has nothing left to randomise and **refuses the draw** rather than returning the observation as its own null. By the standing rubric that makes every OpeningRange number a baseline and not a finding — `docs/roadmap.md` §M28.1.
